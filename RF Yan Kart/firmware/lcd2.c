...
void lcd_komut(char komut)
{
		...
		GPIO_WriteBit(GPIOC,D4,0);
 		GPIO_WriteBit(GPIOC,D3,0);
		GPIO_WriteBit(GPIOC,D2,0);
		GPIO_WriteBit(GPIOC,D1,0);
		GPIO_WriteBit(GPIOC,D0,0);
		...
		if(komut & 0x08)
				GPIO_WriteBit(GPIOC,D3,1);
		if(komut & 0x04)
				GPIO_WriteBit(GPIOC,D2,1);
		if(komut & 0x02)
				GPIO_WriteBit(GPIOC,D1,1);
		if(komut & 0x01)
				GPIO_WriteBit(GPIOC,D0,1);			//GPIOC->ODR |= (komut & 0x000000F0);		
...
{
    unsigned long delay=0x0007FFFF;
    while(delay--);
}